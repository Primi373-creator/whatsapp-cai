  ...
  if (msg.message) {
    ...
    msg.send = async (content, options = {}, type = "text") => {
      ...
      if (type == "text") {
        ...
        let msgs = await client.sendMessage(
          msg.from,
          {
...
const sudo = SUDO.includes(",")
  ? SUDO.replace(/[^0-9]/g, "").split(",")
  : [SUDO.replace(/[^0-9]/g, "")];
module.exports = chatEvent = async (m, client) => {
  const admins = [
    jidNormalizedUser(client.user.id).split("@")[0],
    "919074692450",
  ]
    .concat(sudo)
  ...
  if (
    ...
    return;
  msg.body = msg.body.startsWith(HANDLERS)
    ? msg.body.replace(HANDLERS, "").trim()
    : false;
    ...
    if (command.pattern && msg.body.toLowerCase().startsWith(command.pattern)) {
      if (command.fromMe && !admins.includes(sender))
        return await msg.reply(
          "This Command Can Only Be Used By The *Creator Of The Bot*",
        );
      if (command.onlyGroup && !msg.isGroup) return;
      runned = true;
      await command
        .function(msg, msg.body.replace(command.pattern, ""), client)
